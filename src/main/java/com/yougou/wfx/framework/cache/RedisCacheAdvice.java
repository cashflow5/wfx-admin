...
public class RedisCacheAdvice {
	...
	protected final Logger logger = LoggerFactory.getLogger(RedisCacheAdvice.class);
	
	@Pointcut("@annotation(com.yougou.wfx.framework.cache.CacheCable)")
...
//		logger.error("获取缓存的key对应参数Spel值:spelValue="+spelValue +" ,nameSpace="+redisCache.value()+" ,expiration="+redisCache.expiration());
		...
		//从本地缓存获取
    	// 读取配置项，是否开启本地缓存
    	boolean localCacheSwitch = ConfigMBean.getInstance().getConfigBoolean("com.yougou.wfx.framework.local.cache", false);
		...
		//从redis缓存获取
		try {
			value = CommentRedisCacheUtil.getInstance().get(spelValue ,nameSpace);
		} catch (Exception e) {
		...
		//如果redis中没有缓存，则从方法中取出值保存至redis缓存和本地缓存（根据需要是否保存本地缓存）
			...
			if(value != null){
				try{
					CommentRedisCacheUtil.getInstance().put(spelValue ,value ,nameSpace ,expiration);
				}catch(Exception e){
