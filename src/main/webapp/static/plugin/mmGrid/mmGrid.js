...
!function($){
    ...
    MMGrid.prototype = {
        ...
        _initLayout: function($el){
            ...
            if(opts.isSubGrid){
            	mmGrid = [
        	          '<div style="font-size:11px;-webkit-border-radius: 4px;-moz-border-radius: 4px;border-radius: 4px;" class="mmGrid">',
                          '<style></style>',
                          '<div class="mmg-headWrapper" style="background: #fff;filter: none;-moz-box-shadow: none; -webkit-box-shadow: none;box-shadow: none;border-top: 0;">',
                              '<table class="mmg-head" cellspacing="0"></table>',
                          '</div>',
                          '<div class="mmg-colResizePointer"></div>',
                          '<div class="mmg-colResizePointer-before"></div>',
            ...
            if(opts.checkCol){
                //修复列设置显示隐藏错误问题 update by guoran 20160524
                //如果去掉<label class="lbl"></label> 会将checkbox重写样式删除。而ace改写了默认的checkbox使其隐藏，导致所有checkbox不可见
                var chkHtml = opts.multiSelect ?  '<input type="checkbox" class="checkAll ace ace-checkbox-1" ><span for="" class="lbl"></span>'
                    : '<input type="checkbox" disabled="disabled" class="checkAll ace ace-checkbox-1"><span for="" class="lbl"></span>';
                opts.cols.unshift({title:chkHtml,width: 27, align: 'center' ,lockWidth: true, checkCol: true, renderer:function(){
                    return '<input type="checkbox" class="mmg-check ace ace-checkbox-1"><label for="" class="lbl"></label>';
                }});
            }
            ...
            if(opts.subGridCol && opts.subGridRowExpanded != null){
            	var html = [];
            	html.push('<a class="headerSubGridLink" style="cursor:pointer;">');
            	if(opts.isSubGridExpanded){
                ...
                opts.cols.unshift({title:html.join(''),width: 23, align: 'center' ,lockWidth: true, subGridCol:true, renderer:function(){
                    return '<a class="subGridLink" name="subGridLink_'+ $this._id +'" style="cursor:pointer;"><span class="fa fa-plus" name="subGridLinkBtn_'+ $this._id +'"></span></a>';
                }});
            }
        ...
        , _initOptions: function(){
            ...
            if(cols){
                ...
                //bbHtml.push('<a class="mmg-btnBackboardUp" style="right:5;top:0;"></a>');
                for(var colIndex=0; colIndex<cols.length; colIndex++){
                    bbHtml.push('<label ');
                    if(cols[colIndex].checkCol || cols[colIndex].indexCol || cols[colIndex].subGridCol){
        ...
        , _initEvents: function(){
            ...
            //向下按钮
            var $btnBackboardDn = $mmGrid.find('a.mmg-btnBackboardDn').on('click', function(e){
...
//                    $mmGrid.height($headWrapper.outerHeight(true) + $backboard.outerHeight(true));
            ...
            $(document).click(function(e) {
            	if(!$(".mmg-backboard").is(":hidden") && e.target != $(".mmg-backboard")[0] && e.target != $(".mmg-btnBackboardUp")[0]) {
            		var elm = e.target || e.srcElement;
            		var tagName = elm.tagName.toLowerCase();
            		if(tagName != "span" && tagName != "label" && tagName != "h1"){
            			$mmGrid.find('a.mmg-btnBackboardUp').click();
            		}
            	}
            });
...
//                    $bodyWrapper.height($mmGrid.height() - $headWrapper.outerHeight(true));
            ...
            $body.on('click','td',function(e){
                ...
                var index = parseInt($this.parent().attr('data_index'));
            ...
            $body.on('click','tr > td .mmg-check',function(e){
                e.stopPropagation();
                var $this = $(this);
                var index = parseInt($($this.parent().parent().parent()).attr('data_index'));
                if(this.checked){
                	that.select(index);
                }else{
                	that.deselect(index);
            ...
            if(opts.subGridCol && opts.subGridRowExpanded != null){
	            var $this = this;
	            this.$body.on('click','.subGridLink',function(e){
	            	e.stopPropagation();
	            	var item = $.data($(this).parent().parent().parent()[0], 'item');
	            	
	            	var colspan = $this.$head.find('th:visible').length;
	            	var curTr = $($(this).parent().parent().parent()[0]);
	            	var spanObj = $($(this).children()[0]);
	            	
	            	var newSubGridTr = curTr.next();
	            	// 展开
	            	if(spanObj.hasClass('fa-plus')){
	            		if(newSubGridTr.attr("name") == "childTr"){
	            			spanObj.removeClass('fa-plus').addClass('fa-minus').removeClass('fa-spin');
	            			newSubGridTr.show();
	            			return;
	            		}
	            		var id = new Date().getTime();
	            		var subGridTable = "subGridDiv_"+id;
	            		var subGridPager = "subGridPager_"+id;
	            		var html = [];
	            		html.push('<tr class="childTr" name="childTr">');
	            		html.push('<td class="childTd" colspan="'+ colspan +'">');
	            		html.push('<div style="padding-left:18px;">');
	            		html.push('<table id="'+ subGridTable +'" class="mmg"></table><div id="'+ subGridPager +'" style="text-align:left;font-size:11px;"></div></div>');
	            		html.push('</td>');
	            		html.push('<tr>');
	            		curTr.after(html.join(''));
	            		// loadding
	            		spanObj.addClass('fa-spin');
	            		$(".childTr").hover(function(){
	        				e.stopPropagation();
	        			},function(){
	        				e.stopPropagation();
	        			}).click(function(e){
	        			...
	        			var subGrid = opts.subGridRowExpanded(e,item,subGridTable,subGridPager);
	        			subGrid.on("loadSuccess",function(){
	        				spanObj.removeClass('fa-plus').addClass('fa-minus').removeClass('fa-spin');
	        			});
	            	}else{// 收起
	            		spanObj.removeClass('fa-minus').addClass('fa-plus');
	            		newSubGridTr.hide();
	            	}
	            });
	            
	            this.$head.on('click','.headerSubGridLink',function(e){
	            	var spanObj = $($(this).children()[0]);
	            	e.stopPropagation();
	            	// 展开
	            	if(spanObj.hasClass('fa-plus')){
            			spanObj.removeClass('fa-plus').addClass('fa-minus').removeClass('fa-spin');
            			$('a[name="subGridLink_'+$this._id+'"]').each(function(index,element){
            				var spanObj = $($(this).children()[0]);
    	            		if(spanObj.hasClass('fa-plus')){
    	            			$(this).click();
    	            		}
        ...
        , _rowHtml: function(item, rowIndex){
            ...
            if($.isPlainObject(item)){
                var trHtml = [];
                trHtml.push('<tr class="parentTr" data_index="'+rowIndex+'">');
        ...
        , _insertEmptyRow: function(){
            var $body = this.$body;
            $body.empty().html('<tbody><tr class="emptyRow"><td style="border: 0px;background: none;text-align:center">没有数据</td></tr></tbody>');
        }
        ...
        , _setColsWidth: function(){
            ...
            for(var colIndex=0; colIndex<$ths.length; colIndex++){
                ...
                if(col.position){
                	styleText.push('vertical-align: '+col.position+';');
                }
                if(col.hidden){
        ...
        , _showLoading: function(){
            ...
            if(this.opts.fullScreenLoading){
	            YouGou.UI.loading();
	            YouGou.UI.loadingClickEvent();
            }
        }
        ...
        , _loadAjax: function(args){
            ...
            if(opts.remoteSort){
                ...
                if(sortName){
                    params.orderSort = sortName+'.'+sortStatus;
                }
            }
            ...
            $.ajax({
                ...
                // 是否自动展开子表格
                if(opts.isSubGridExpanded){
                	$.each($('.subGridLink'), function(index){
                        var $this = $(this);
                        if($this.children().eq(0).hasClass("fa-plus")){
                        	$this.click();
                        }
                    });
                }
                ...
                if(statusCode == 500){
                	...
                	message = data.responseText.replace(/(\n)+|(\r\n)+/g, "");
                }else if(statusCode == 404){
                	title = '404，您访问的列表数据地址未找到！';
                	var html = [];
                	html.push('<div class="row"><div class="col-xs-12"><div class="error-container"><div class="well"><h1 class="grey lighter smaller"><span class="bigger-125" style="color: #3DA8B9 !important;"><i class="ace-icon fa fa-sitemap"></i>404</span>您访问的列表数据地址未找到！</h1></div></div></div></div>');
                	message = html.join('');
                	...
                	title = '请求参数可能错误！';
                	var html = [];
                	html.push('<div class="row"><div class="col-xs-12"><div class="error-container"><div class="well"><h1 class="grey lighter smaller"><span class="bigger-125" style="color: #3DA8B9 !important;"><i class="ace-icon fa fa-sitemap"></i>400</span>请求参数可能错误！</h1></div></div></div></div>');
                	message = html.join('');
                }
        ...
        , _loadNative: function(args){
            ...
            // 是否自动展开子表格
            if(this.opts.isSubGridExpanded){
            	$.each($('.subGridLink'), function(index){
                    var $this = $(this);
                    if($this.children().eq(0).hasClass("fa-plus")){
                    	// 如果是本地数据加载立马展开
        ...
        , select: function(args){
            ...
            var $tr = $body.children().children().filter("[data_index='"+ args +"']");
            var subGrid = $tr.next();
            
            if(subGrid.hasClass('childTr')){
            	subGrid.find('tr').addClass('selected');
                subGrid.find('.mmg-check').prop('checked','checked');
                subGrid.find('.checkAll').prop('checked','checked');
            }
            ...
            if(typeof args === 'number'){
                ...
                $body.find('.checkAll').prop('checked','checked');
                $body.parent().parent().parent().parent().parent().prev().find('td .mmg-check').prop('checked','checked');
            }else{
                return;
            }
        ...
        , deselect: function(args){
            ...
            if(typeof args === 'number'){
                ...
                var subGrid = $tr.next();
                if(subGrid.hasClass('childTr')){
                	subGrid.find('.mmg-check').prop('checked','');
                	subGrid.find('.checkAll').prop('checked','');
                	subGrid.find('.selected').removeClass('selected');
                	subGrid.find("tr").removeClass('selected');
                }
                $tr.removeClass('selected');
                subGrid.removeClass('selected');
                if(opts.checkCol){
                	$tr.find('.mmg-check').prop('checked','');
                }
            }else if(typeof args === 'function'){
                $.each($body.find('tr'), function(index){
                    if(args($.data(this, 'item'), index)){
                ...
                $body.find('tr.selected').removeClass('selected');
                $body.find('.checkAll').prop('checked','');
                if(opts.checkCol){
                    $body.find('tr > td').find('.mmg-check').prop('checked','');
                    $body.parent().parent().parent().parent().parent().prev().find('td .mmg-check').prop('checked','');
                }
            }else{
                return;
            }
        ...
        , selectedRows: function(){
            var $body = this.$body;
            var selected = [];
            $.each($body.children().children().filter('tr.selected.parentTr'), function(index ,item){
        ...
        , setAutoHeight : function(){
    		...
    		$(".mmGrid,.mmg-bodyWrapper").css("height","auto");
        }
    };
