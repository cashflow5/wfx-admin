...
var actionFixed = function(val,item,rowIndex){
	...
	if(isOnsale==1){
		html.push("<a  href=\"javascript:void(0);\"  onclick=\"updateShelvesStatus(\'"+id+"\',2,\'"+costPrice+"\',\'"+wfxCostPrice+"\',\'"+wfxPrice+"\',\'"+no+"\');\">下架</a>&nbsp;&nbsp;");
	}else if(isOnsale==2 || isOnsale==3){
		if(wfxCostPrice!=0  &&  wfxPrice!=0){
			html.push("<a  href=\"javascript:void(0);\"  onclick=\"updateShelvesStatus(\'"+id+"\',1,\'"+costPrice+"\',\'"+wfxCostPrice+"\',\'"+wfxPrice+"\',\'"+no+"\');\">上架</a>&nbsp;&nbsp;");
		}
...
//修改上下架状态
function updateShelvesStatus(id,shelvesStatus,costPrice,wfxCostPrice,wfxPrice,no){
	
	var titleName="";
	if(shelvesStatus==1){
		if(!checkShelves(costPrice,wfxCostPrice,wfxPrice,no)){
			return ;
		}
		titleName="确定要上架商品吗？";
	}else{
...
function batchCheckShelves(costPrice,wfxCostPrice,wfxPrice,no){
	var pointOut='';
	if(wfxCostPrice==null || wfxCostPrice==undefined ||  wfxCostPrice<=0){
		if(pointOut==''){
			pointOut=no+":";
		}
		pointOut=pointOut+"微分销成本价不能小于0,";
	}
	if(wfxPrice==null || wfxPrice==undefined ||  wfxPrice<=0){
		if(pointOut==''){
			pointOut=no+":";
		}
		pointOut=pointOut+"微分销价不能小于0,";
	}
	...
	if(wfxCostPrice<costPrice){
		...
		pointOut=pointOut+"微分销成本价不能小于优购成本价,";
	...
	if(wfxPrice<wfxCostPrice){
		...
		pointOut=pointOut+"微分销销售价不能小于微分销成本价,";
	}
	if(pointOut!=''){
		pointOut=pointOut+"</br>";
	}
	return pointOut;
}

function checkShelves(costPrice,wfxCostPrice,wfxPrice,no){
	var pointOut='';
	if(wfxCostPrice==null || wfxCostPrice==undefined ||  wfxCostPrice<=0){
		pointOut=no+"微分销成本价不能小于0";
		YouGou.UI.Dialog.alert({message:pointOut});
		return false;
	}
	if(wfxPrice==null || wfxPrice==undefined ||  wfxPrice<=0){
		pointOut=no+"微分销价不能小于0";
	...
	wfxPrice=wfxPrice-0;
	if(wfxCostPrice<costPrice){
		pointOut=no+"微分销成本价不能小于优购成本价";
		YouGou.UI.Dialog.alert({message:pointOut});
		return false;
	}
	if(wfxPrice<wfxCostPrice){
		pointOut=no+"微分销销售价不能小于微分销成本价";
		YouGou.UI.Dialog.alert({message:pointOut});
		return false;
	}
	return true;
...
function batchUpdateShelvesStatus(shelvesStatus){
	...
	if(checkObjs.length > 0){
		checkObjs.each(function(){
			...
			var no = spans.eq(size-9).text();
			pointOut=pointOut+batchCheckShelves(costPrice,wfxCostPrice,wfxPrice,no);
   		...
   		dataType:"json",
   	  	...
   	  	success : function(data){
   	  		if(data.data.resultCount >0 ){
    	  		mmGrid2.load();
    	  	}else if( data.data.resultCount ==-1){
       	  		YouGou.UI.Dialog.alert({message:data.msg+"：佣金比例为0的商品不能上架"});
   	  		}else{
...
function readjustPrices(publicPrice ,salePrice,id,costPrice,wfxCostPrice,wfxPrice,distributableMoney){
	...
	YouGou.UI.Dialog.show({
		...
		message: function(dialog) {
			var msg = '<div>市场价：'+publicPrice+' <br/>优购价：'+salePrice+'</br>微分销成本价：<input id="wfxCostPrice" value=\"'+wfxCostPrice+'\"   onmouseout=\"wfxFormula1('+publicPrice+');\" /></br>微分销零售价：<input id="wfxPrice"     value=\"'+wfxPrice+'\"  onmouseout=\"wfxFormula1('+publicPrice+');\"   /></div>';
			return msg;
		},
		buttons: [
		    {
...
var cols = [
   ...
   { title:'真实成本', name:'costPrice',width:60, align:'left', hidden: true,lockDisplay:true},
