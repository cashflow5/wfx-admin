    ...
    function LoadTree(znodes, container, isFlag) {
        var setting = {
            async: {
                enable: true,
                dataType:'json',
                url: "/basicSet/getAreaJsonForZtree.sc",
        ...
        function beforeRemove(treeId, treeNode) {
            ...
            if (treeNode.isParent) {
                ...
                if(window.confirm("此节点存在下级节点，确认要删除节点（" + treeNode.name + " ）及其下级节点吗？")){
            		$.ajax({
                        url: '/basicSet/removeAreaMgt.sc',
                        type: 'post',
                        dataType: 'json',
                        async: false,
                        data: {
                    	...
                    	if(data.state == 'error'){
                    		...
                    		YouGou.UI.Dialog.autoCloseTip("删除成功！");
                    	}
                    });
            	}else{
            		flag = false;
            	...
            	if(window.confirm("确认要删除节点（" + treeNode.name + " ）吗？")){
            		$.ajax({
                        url: '/basicSet/removeAreaMgt.sc',
                        type: 'post',
                        dataType: 'json',
        ...
        function beforeRename(treeId, treeNode, newName, isCancel) {
            ...
            if (!isCancel && treeNode.name != newName && isValid) {
        		YouGou.UI.Dialog.autoCloseTip('该层级的节点名称重复，请重新输入！');
                return false;
            }
            return true;
        }
        ...
        function onNodeRename(event, treeId, treeNode) {
            ...
            $.ajax({
                url: '/basicSet/updateAreaMgt.sc',//重命名
                type: 'post',
                dataType: 'json',
                data: {
                    id: treeNode.id,
        ...
        function onClick(event, treeId, treeNode) {
            ...
            console.log('Name:' + treeNode + ' ID:' + treeNode.id);
            if (treeNode.level == 0)
                return;
