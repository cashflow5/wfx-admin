...
<script>
   	function bindCopy(){
   		var zCopyObj = $('.zCopy');
   		zCopyObj.parent().parent().css('position', 'relative');
		$('.zCopy').zclip({
	        path: '/static/plugin/zeroclipboard/ZeroClipboard.swf',
	        copy: function() { //复制内容 
   				var commNo = $(this).attr('data-no');
	        	var linkUrl = h5Domain + '/yougoushop/item/' + commNo + '.sc';
	            return linkUrl;
	        },
    ...
    (function () {
            ...
            function Init() {
           	...
           	function copyRender(val){
           		var html = '<a class="zCopy" data-no="'+val+'" href="javascript:void(0);">复制链接</a>';
           		return html;
           	}
            ...
            function mmgridTable() {
                ...
                var cols = [
                    ...
                    {title: '操作', name: 'no', align: 'center', sortable: true, width: 80, lockWidth: true,
                    renderer:copyRender}
                ],
